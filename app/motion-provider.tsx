...
export default function MotionProvider() {
  ...
  useEffect(() => {
    ...
    // Portfolio images/webfonts often finish loading after ScrollTrigger has
    // already calculated trigger positions from the pre-load layout. Once
    // everything has actually loaded, the page height/element offsets can
    // shift enough that a scroll position which should fire `onEnter` for a
    // `once: true` trigger never does — leaving that element's autoAlpha:0
    // starting state in place forever. Recalculating after `load` (and once
    // more shortly after, for late webfont swaps) keeps trigger positions
    // honest without needing to touch every place ScrollTrigger.create() is
    // called above.
    ...
    const lateRefreshTimer = window.setTimeout(refresh, 1200);
