...
function safeFileSegment(input: string): string {
  return (
    input
      ...
      // Dots are allowed (e.g. "file.png"), but a bare ".." run has no
      // legitimate use in a slug/filename and is exactly the substring a
      // path-traversal attempt relies on — collapse it to a single dash so
      // it can never appear, even though resolveOnDisk() already refuses
      // to escape STORAGE_ROOT regardless (defense in depth, not the only
      // line of defense).
      .replace(/\.{2,}/g, "-")
      .replace(/-+/g, "-")
      .slice(0, 80)
...
/** Creates a project's (empty) media folder immediately on project creation, so it exists on disk before any upload. */
