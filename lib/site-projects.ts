...
function safeParseObject(json: string): Record<string, Record<string, unknown>> {
  try {
    const value = JSON.parse(json);
...
export async function getAdminProjects(locale: Locale): Promise<Project[]> {
  ...
  for (const row of rows) {
    ...
    result.push({
      slug: row.slug,
      title: str(localeData.title) || row.slug,
      services: str(localeData.services),
      image,
      categories,
...
export type AdminProjectDetail = {
  ...
  /** Raw per-locale content object — title/services/challenge/approach plus
   * any premium fields (heroLine1, chips, cards, metrics, stages, links…)
   * filled in from the admin panel's "Premium layout" section. */
...
export async function getAdminProjectDetail(slug: string, locale: Locale): Promise<AdminProjectDetail | null> {
  ...
  return {
    ...
    locale: { ...localeData, title: str(localeData.title) || row.slug },
  };
}
...
export async function getAllProjects(locale: Locale): Promise<Project[]> {
  ...
  // the hand-built static case studies follow. If an admin project reuses
  // the slug of a hand-built one (e.g. the seeded "keyman-chicago"), the
  // admin/DB version wins and the static duplicate is dropped — otherwise
  // the same case study would render twice in the Work grid.
  const dbSlugs = new Set(dbProjects.map((p) => p.slug));
  const remainingStatic = staticProjects.filter((p) => !dbSlugs.has(p.slug));
  return [...dbProjects, ...remainingStatic];
}